...
mixed integer solution *)
  ...
  *)
  ...
  module Mpscp = struct
    ...
    let obj_name = field t "obj_name" char
...
(* ERREUR
   ...
   foreign "glp_ios_add_row"
    (Tree.t @-> char @-> int @-> int @-> int @-> int @-> float @-> int @-> float @-> returning int )
   in
...
let read_mps lp fmt mp = 
  let go =
    foreign "glp_read_mps"
      (problem @-> int @-> Mpscp.t @-> char @-> returning int )
  in 
  go lp (of_fmt fmt)
...
let read_lp =
  foreign "glp_read_lp"
    (problem @-> Cpxcp.t @-> char @-> returning int )
...
let mpl_read_model =
  foreign "glp_mpl_read_model"
    (Tran.t @-> char @-> int @-> returning int)

(*int glp_mpl_read_data(glp_tran *tran, const char *fname);*)
let mpl_read_data =
  foreign "glp_mpl_read_data"
    (Tran.t @-> char @-> returning int)
